...
H$ref_TD.hour <- c("TF00C1_60", "TY00Y_60", "AU0_60", "CU0_60", "IH00C1_60")
H$ref_TD.hour.text <- c("5YR TREASURY Current 60 min", "US 10yr Current 60 min", "SHFE GOLD Major 60 min", "SHFE COPPER Major 60 min", "CFFEX A50 Current 60 min")
H$ref_TD.hour.bk <- H$ref_TD.hour
H$ref_TD <- c("TF00C1", "TY00Y", "AU0" , "CU0", "IH00C1")
H$ref_TD.text <- c("CFFEX 5yr Treasury", "US 10yr Treasury", "SHFE GOLD Futures", "SHFE COPPER Futures", "CFFEX A50 Futures")
...
```{r}
#Changed into xts file.
H$DailyTD.TY00Y.xts <- H[["DailyTD.xts.List"]][["TY00Y_hpf.xts"]]
H$HourlyTD.xts.List$TY00Y_60_hpf.2.xts <- align.time(H$HourlyTD.xts.List$TY00Y_60_hpf.xts, 
                                                     n = 9 * 60*60)

#Align daily trade data with hourly trade data.
H$MixFreTD.TY00Y.xts <- merge.xts(H$HourlyTD.xts.List$TY00Y_60_hpf.2.xts,
                                 H$DailyTD.TY00Y.xts,
                                 join = "left")
index(H$MixFreTD.TY00Y.xts) <- index(H$HourlyTD.xts.List$TY00Y_60_hpf.xts)

#Try Using ggplot2 to show up the result.
H$x_label <- seq.POSIXt(from = index(H$MixFreTD.TY00Y.xts)[1], 
                          to = index(H$MixFreTD.TY00Y.xts)[dim(H$MixFreTD.TY00Y.xts)[1]], by = "week")
H$MixFreTD.TY00Y.gg <- ggplot(H$MixFreTD.TY00Y.xts, aes(x = as.factor(index(H$MixFreTD.TY00Y.xts)), group = 1)) +
  geom_line(aes(y = TY00Y_60Close)) + 
  geom_line(aes(y = TY00Y_60_hpft), colour = "red") + 
  geom_point(aes(y = TY00Y_hpft), colour = "blue", na.rm = TRUE) +
  scale_x_discrete(breaks = c(as.character(H$x_label)), labels = format(x = H$x_label, format = "%m%d")) +
  ggtitle("TY00Y Hourly and Daily Trend.") + xlab("Trading Date")
H$MixFreTD.TY00Y.gg
```
...
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
H$GPD.D.fit.tg1 <- gpdFit(H$GPD.D.cycle.tg1, u = 2.0)
H$GPD.D.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.D.fit.tg1, prob = H$prob.ind),
                           digits = 3)
##Output Table
H$GPD.H.VaRES.tg1.gg <- ggtexttable(H$GPD.H.VaRES.tg1, 
...
for(i in 1:ncol(H$dataset_for_plot)){
  H$dataset_for_plot[, i] <- H$dataset_for_plot[, 1] - mean(na.omit(H$dataset_for_plot[, i]))
}
