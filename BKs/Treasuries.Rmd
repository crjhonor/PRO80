...
H$ref_TD.hour <- c("TF00C1_60", "T00C1_60", "ZN0_60", "CU0_60", "IH00C1_60")
H$ref_TD.hour.text <- c("5YR TREASURY Current 60 min", 
                        "10YR TREASURY Current 60 min", 
                        "SHFE ZNIC  Major 60 min", 
                        "SHFE COPPER Major 60 min", 
                        "CFFEX A50 Index Current 60 min")
H$ref_TD.hour.bk <- H$ref_TD.hour
H$ref_TD <- c("TF00C1", "T00C1", "ZN0" , "CU0", "IH00C1")
H$ref_TD.text <- c("CFFEX 5yr Treasury", "CFFEX 10yr Treasury", "SHFE ZNIC Futures", "SHFE COPPER Futures", "CFFEX A50 Index Futures")
...
H$DailyTD.TF00C1.xts <- H[["DailyTD.xts.List"]][["TF00C1_hpf.xts"]]
H$HourlyTD.xts.List$TF00C1_60_hpf.2.xts <- align.time(H$HourlyTD.xts.List$TF00C1_60_hpf.xts, 
                                                     n = 9 * 60*60)

#Align daily trade data with hourly trade data.
H$MixFreTD.TF00C1.xts <- merge.xts(H$HourlyTD.xts.List$TF00C1_60_hpf.2.xts,
                                 H$DailyTD.TF00C1.xts,
                                 join = "left")
index(H$MixFreTD.TF00C1.xts) <- index(H$HourlyTD.xts.List$TF00C1_60_hpf.xts)

#Try Using ggplot2 to show up the result.
H$x_label <- seq.POSIXt(from = index(H$MixFreTD.TF00C1.xts)[1], 
                          to = index(H$MixFreTD.TF00C1.xts)[dim(H$MixFreTD.TF00C1.xts)[1]], by = "week")
H$MixFreTD.TF00C1.gg <- ggplot(H$MixFreTD.TF00C1.xts, aes(x = as.factor(index(H$MixFreTD.TF00C1.xts)), group = 1)) +
  geom_line(aes(y = TF00C1_60Close)) + 
  geom_line(aes(y = TF00C1_60_hpft), colour = "red") + 
  geom_point(aes(y = TF00C1_hpft), colour = "blue", na.rm = TRUE) +
  scale_x_discrete(breaks = c(as.character(H$x_label)), labels = format(x = H$x_label, format = "%m%d")) +
  ggtitle("TF00C1 Hourly and Daily Trend.") + xlab("Trading Date")
H$MixFreTD.TF00C1.gg
```
...
##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 0.2)
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
H$GPD.D.fit.tg1 <- gpdFit(H$GPD.D.cycle.tg1, u = 1.0)
...
##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 0.35)
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
...
##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 1000)
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
H$GPD.D.fit.tg1 <- gpdFit(H$GPD.D.cycle.tg1, u = 3500)
H$GPD.D.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.D.fit.tg1, prob = H$prob.ind),
                           digits = 3)
##Output Table
H$GPD.H.VaRES.tg1.gg <- ggtexttable(H$GPD.H.VaRES.tg1, 
...
H$ref_TD.hour.bk.lwd <- c(rep(1, 1), rep(4, 3), rep(5, 2))
H$ref_TD.hour.bk.lty <- c(rep(1, 1), rep(3, 3), rep(5, 2))
...
H$dataset_for_plot[, 1] <- H$dataset_for_plot[, 1] * 100
H$dataset_for_plot[, 2] <- H$dataset_for_plot[, 2] * 100
H$dataset_for_plot[, 3] <- H$dataset_for_plot[, 3] / 10
H$dataset_for_plot[, 4] <- H$dataset_for_plot[, 4] / 10
H$dataset_for_plot[, 5] <- H$dataset_for_plot[, 5] / 5
