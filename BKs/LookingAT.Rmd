...
H$ref_TD.hour <- c("M0_60", "T00C1_60", "RB0_60", "LH0_60", "RU0_60")
H$ref_TD.hour.text <- c("DCE SoyMeal Major 60 min", "10YR TREASURY Current 60 min", "SHFE REBAR Major 60 min", "DCE PORK Major 60 min", "SHFE RUBBER Major 60 min")
H$ref_TD.hour.bk <- H$ref_TD.hour
H$ref_TD <- c("M0", "T00C1", "RB0" , "LH0", "RU0")
H$ref_TD.text <- c("DCE SoyMeal Futures", "CFFEX 10yr Treasury", "SHFE REBAR Futures", "DCE PORK Futures", "DCE RUBBER Futures")
...
```{r}
#Changed into xts file.
H$DailyTD.LH0.xts <- H[["DailyTD.xts.List"]][["LH0_hpf.xts"]]
H$HourlyTD.xts.List$LH0_60_hpf.2.xts <- align.time(H$HourlyTD.xts.List$LH0_60_hpf.xts, 
                                                     n = 9 * 60*60)

#Align daily trade data with hourly trade data.
H$MixFreTD.LH0.xts <- merge.xts(H$HourlyTD.xts.List$LH0_60_hpf.2.xts,
                                 H$DailyTD.LH0.xts,
                                 join = "left")
index(H$MixFreTD.LH0.xts) <- index(H$HourlyTD.xts.List$LH0_60_hpf.xts)

#Try Using ggplot2 to show up the result.
H$x_label <- seq.POSIXt(from = index(H$MixFreTD.LH0.xts)[1], 
                          to = index(H$MixFreTD.LH0.xts)[dim(H$MixFreTD.LH0.xts)[1]], by = "week")
H$MixFreTD.LH0.gg <- ggplot(H$MixFreTD.LH0.xts, aes(x = as.factor(index(H$MixFreTD.LH0.xts)), group = 1)) +
  geom_line(aes(y = LH0_60Close)) + 
  geom_line(aes(y = LH0_60_hpft), colour = "red") + 
  geom_point(aes(y = LH0_hpft), colour = "blue", na.rm = TRUE) +
  scale_x_discrete(breaks = c(as.character(H$x_label)), labels = format(x = H$x_label, format = "%m%d")) +
  ggtitle("LH0 Hourly and Daily Trend.") + xlab("Trading Date")
H$MixFreTD.LH0.gg
```
...
##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 1200)
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
H$GPD.D.fit.tg1 <- gpdFit(H$GPD.D.cycle.tg1, u = 2800)
H$GPD.D.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.D.fit.tg1, prob = H$prob.ind),
                           digits = 3)
##Output Table
H$GPD.H.VaRES.tg1.gg <- ggtexttable(H$GPD.H.VaRES.tg1, 
...
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
H$GPD.D.fit.tg1 <- gpdFit(H$GPD.D.cycle.tg1, u = 2500)
H$GPD.D.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.D.fit.tg1, prob = H$prob.ind),
                           digits = 3)
##Output Table
H$GPD.H.VaRES.tg1.gg <- ggtexttable(H$GPD.H.VaRES.tg1, 
...
H$ref_TD.hour.bk.lwd <- c(rep(3, 1), rep(4, 2), rep(5, 3))
H$ref_TD.hour.bk.lty <- c(rep(1, 1), rep(3, 2), rep(5, 3))
...
H$dataset_for_plot[, 1] <- H$dataset_for_plot[, 1] / 1
...
H$dataset_for_plot[, 5] <- H$dataset_for_plot[, 5] / 10
