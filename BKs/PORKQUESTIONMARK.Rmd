...
H$ref_TD.hour <- c("LH0_60", "T00C1_60")
H$ref_TD.hour.text <- c("DCE PORK Major 60 min", "10YR TREASURY Current 60 min")
H$ref_TD.hour.bk <- H$ref_TD.hour
H$ref_TD <- c("LH0", "T00C1")
H$ref_TD.text <- c("DCE PORK Futures", "CFFEX 10yr Treasury")
...
#Fetching Dataset.
H$DIST.Dataset.D <- H$DataSet_Fetch_D(H$ref_TD[H$ind], "_hpf.xts") / c(10, 10, 10)
...
legend("topleft", legend = c("empirical(X10)", "GHD(X10)"), col = H$DIST.D.col.def, lty = 1)
...
H$GPD.D.cycle.tg1 <- H$DIST.D.ycycle * 10

##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 950)
...
##GPD
H$GPD.H.fit.tg1 <- gpdFit(H$GPD.H.cycle.tg1, u = 12)
H$GPD.H.VaRES.tg1 <- round(gpdRiskMeasures(H$GPD.H.fit.tg1, prob = H$prob.ind),
                           digits = 3)
...
H$dataset_for_plot[, 1] <- H$dataset_for_plot[, 1] / 20
H$dataset_for_plot[, 2] <- H$dataset_for_plot[, 2] * 100
H$dataset_for_plot[, 3] <- H$dataset_for_plot[, 3]
