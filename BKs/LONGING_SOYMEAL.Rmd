...
H$ref_TD.hour <- c("M0_60", "T00C1_60", "P0_60", "Y0_60", "IH00C1_60")
H$ref_TD.hour.text <- c("DCE Soymeal Major 60 min", 
                        "10YR TREASURY Current 60 min", 
                        "DCE Palmoil Major 60 min", 
                        "DCE Soyoil Major 60 min", 
                        "CFFEX A50 Index Current 60 min")
H$ref_TD.hour.bk <- H$ref_TD.hour
H$ref_TD <- c("M0", "T00C1", "P0" , "Y0", "IH00C1")
H$ref_TD.text <- c("DCE Soymeal Futures", "CFFEX 10yr Treasury", "DCE Palmoil Futures", "DCE Soyoil Futures", "CFFEX A50 Index Futures")
...
```{r}
#Changed into xts file.
H$DailyTD.Y0.xts <- H[["DailyTD.xts.List"]][["Y0_hpf.xts"]]
H$HourlyTD.xts.List$Y0_60_hpf.2.xts <- align.time(H$HourlyTD.xts.List$Y0_60_hpf.xts, 
                                                     n = 9 * 60*60)

#Align daily trade data with hourly trade data.
H$MixFreTD.Y0.xts <- merge.xts(H$HourlyTD.xts.List$Y0_60_hpf.2.xts,
                                 H$DailyTD.Y0.xts,
                                 join = "left")
index(H$MixFreTD.Y0.xts) <- index(H$HourlyTD.xts.List$Y0_60_hpf.xts)

#Try Using ggplot2 to show up the result.
H$x_label <- seq.POSIXt(from = index(H$MixFreTD.Y0.xts)[1], 
                          to = index(H$MixFreTD.Y0.xts)[dim(H$MixFreTD.Y0.xts)[1]], by = "week")
H$MixFreTD.Y0.gg <- ggplot(H$MixFreTD.Y0.xts, aes(x = as.factor(index(H$MixFreTD.Y0.xts)), group = 1)) +
  geom_line(aes(y = Y0_60Close)) + 
  geom_line(aes(y = Y0_60_hpft), colour = "red") + 
  geom_point(aes(y = Y0_hpft), colour = "blue", na.rm = TRUE) +
  scale_x_discrete(breaks = c(as.character(H$x_label)), labels = format(x = H$x_label, format = "%m%d")) +
  ggtitle("Y0 Hourly and Daily Trend.") + xlab("Trading Date")
H$MixFreTD.Y0.gg
```
